...
def build_main_area(window):
    ...
    """
    # Main container is now window.vbox
    main_vbox = window.vbox

    # Top controls
    top_controls = Gtk.Box(spacing=4)
    top_controls.set_border_width(4)
    top_controls.set_margin_bottom(0)
    main_vbox.pack_start(top_controls, False, False, 0)
    ...
    main_vbox.pack_start(window.stack, True, True, 0)
    ...
    lbl = Gtk.Label()
    lbl.set_markup(
        "<span size='xx-large' weight='bold' alpha='50%'>Drop Videos Here</span>"
    )
